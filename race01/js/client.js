window.addEventListener('DOMContentLoaded', function() {
    ...
    function connect() {
        ...
        if (socket === undefined || socket.readyState !== 1) {
            socket = new WebSocket("ws://10.11.10.4:3005");
        } else {
        ...
        socket.onmessage = function(event) { // при получении данных от сервера
            ...
            if (event.data == "Game found") {
                window.location.href = 'views/battle.html';
    ...
    document.getElementById('find').onclick = function() {
        if (socket !== undefined && socket.readyState === 1) {
            ...
            socket.send(message + "|" + document.getElementById('VAL_LOGIN').value);
            showMessage('Отправлено сообщение серверу: ' + message + "|" + document.getElementById('VAL_LOGIN').value);
    ...
    document.getElementById('reg').onclick = function() {
        if (socket !== undefined && socket.readyState === 1) {
            var message = "find";
            socket.send(message + "|" + "cur_user_login");
            showMessage('Отправлено сообщение серверу: ' + message + "|" + "cur_user_login");
